...
document.addEventListener('DOMContentLoaded', () => { if (document.getElementById('employee-search')) document.getElementById('employee-search').addEventListener('input', window.filterEmployees); });
...
window.openEmployeeModal = function(empId) {
    ...
    const empOrders = window.ordersData.filter(o => o.worker && o.worker.includes(emp.name)); document.getElementById('modal-emp-orders-count').innerText = empOrders.length;
...
window.openEmployeeForm = function(empId = null) {
    ...
    if (empId) { const emp = window.employeesData.find(e => e.id === empId); if (emp) { document.getElementById('form-emp-name').value = emp.name; document.getElementById('form-emp-phone').value = emp.phone; document.getElementById('form-emp-address').value = emp.address || ''; document.getElementById('form-emp-birth').value = emp.birthDate || ''; document.getElementById('form-emp-type').value = emp.type; document.getElementById('form-emp-exp').value = emp.exp ? emp.exp.split('/')[0].trim() : ''; document.getElementById('form-emp-access-key').value = emp.accessKey || ''; } window.closeEmployeeModal(); } 
...
window.saveEmployeeForm = function(event) {
    ...
    if (window.currentEditingEmpId) { const emp = window.employeesData.find(e => e.id === window.currentEditingEmpId); if (emp) { emp.name = name; emp.phone = phone; emp.address = address; emp.birthDate = birthDate; emp.type = type; emp.exp = exp; emp.accessKey = accessKey; } } 
    else { window.employeesData.push({ id: window.generateEmpId(), status: 'active', name: name, type: type, phone: phone, exp: exp || '0', rating: 0.0, birthDate: birthDate, address: address, accessKey: accessKey, companyDebt: 0, workingDates: [] }); }
