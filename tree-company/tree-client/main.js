// 1. ЖЕСТКАЯ ОЧИСТКА КЭША ПРИ ЗАПУСКЕ (Чтобы изменения применялись мгновенно)
if ('caches' in window) {
...
// 2. РЕГИСТРАЦИЯ PWA (Без бесконечных перезагрузок)
...
document.addEventListener('DOMContentLoaded', () => {
    try {
        ...
        function updateThemeIcon() {
            if (!themeIcon) return;
            const isDark = body.classList.contains('force-dark') || (!body.classList.contains('force-light') && systemPrefersDark);
        ...
        // Запуск языков и авторизации
        applyLanguage();
        checkClientAuth();

        // --- ЛОГИКА ФОРМ (ЗАКАЗЫ И ВАКАНСИИ) ---
        const applyForm = document.getElementById('apply-form');
        if (applyForm) {
