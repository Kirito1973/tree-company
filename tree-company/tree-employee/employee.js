...
const COMPANY_FEE_PERCENT = 0.10; 
...
const translations = {
    ...
    "login_sub_welcome": { "AM": "Մուտքագրեք Ձեր գաղտնաբառը", "RU": "Введите ваш пароль для доступа к системе", "EN": "Enter your password to access the system" },
    ...
    "login_desc": { "AM": "Մուտքագրեք Ձեր գաղտնաբառը", "RU": "Введите пароль", "EN": "Enter password" },
    ...
    if (loginBtn) {
        loginBtn.addEventListener('click', () => {
            ...
            if (pinInput && pinInput.value) {
                window.loginEmployee(pinInput.value);
            }
        });
    }
...
window.initApp = async function() {
    ...
    if (savedEmpId) {
        loggedInEmpId = savedEmpId;
        await window.fetchEmployeeAppDatabase(loggedInEmpId);
    } else {
...
window.loginEmployee = async function(pin) {
    ...
    try {
        ...
        if (data.success) {
            loggedInEmpId = data.employee.id;
...
window.fetchEmployeeAppDatabase = async function(empId) {
    try {
        ...
        if (res.ok) {
            ...
            window.ordersData = dbData.orders || [];
            window.employeesData = dbData.employees || [];
...
window.renderEmployeeNews = function() {
    ...
    newsSection.innerHTML = `
        ...
        <div class="glass-panel" style="margin-top: 16px; text-align: center; padding: 16px;">
            ...
            <div style="font-size: 38px; font-weight: 900; color: var(--tree-light); line-height: 1;">${(emp.rating || 0).toFixed(1)}</div>
...
window.renderEmployeeFinance = function() {
    ...
    let totalAllTime = 0, currentMonthTotal = 0, uncompletedTotal = 0, companyDebt = emp.companyDebt || 0;
    const monthlyData = {};
...
window.renderEmployeeOrders = function() {
    ...
    empOrders.forEach(order => {
        ...
        let mainTitle = (order.services && order.services.length > 0) ? order.services[0].name : "Услуга";
...
window.openOrderModal = function(orderId) {
    ...
    if (financeBlock && financeBlock.parentNode) {
...
window.confirmSplitAndFinish = async function() {
...
window.renderEmployeeProfile = function(emp) {
    ...
    let pType = emp.typeLabel ? emp.typeLabel.split(' / ') : ['---'];
...
window.saveEmpSelfEdit = async function(e) {
    e.preventDefault();
    const emp = employeesData.find(e => e.id === loggedInEmpId);
    if(!emp) return;
...
window.toggleScheduleDropdown = function() {
    ...
    if (content.classList.contains('open')) {
        ...
        content.classList.add('open'); chevron.style.transform = 'rotate(180deg)'; window.renderCalendar();
    }
};
...
window.saveWeeklySchedule = async function() {
    ...
    setTimeout(() => { btn.innerHTML = originalText; btn.style.background = ''; window.toggleScheduleDropdown(); }, 1000);
};
